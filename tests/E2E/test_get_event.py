...
@pytest.mark.asyncio
async def test_get_event_returns_event(client,
                                       get_event_use_case,
    ...
    expected_response = EventResponse(id=event_id,
                                      name='Python Conference',
                                      place=PlaceResponse(id=place_id,
                                                          name='Conference Hall',
                                                          city='Helsinki',
                                                          address='Mannerheimintie 1',
                                                          seats_pattern='AAAA'),
...
@pytest.mark.asyncio
async def test_get_event_returns_404_when_event_not_found(client,
                                                          get_event_use_case):
    event_id = uuid4()

    get_event_use_case.execute.side_effect = EventNotFoundException(event_id)

    response = await client.get(f'/api/events/{event_id}')

    assert response.status_code == 404
    assert response.json() == {'detail': f'Event {event_id} is not found'}
