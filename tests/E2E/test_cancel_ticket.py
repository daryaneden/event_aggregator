...
@pytest.mark.asyncio
async def test_cancel_ticket_returns_success(
    client,
...
@pytest.mark.asyncio
async def test_cancel_ticket_returns_404_when_ticket_not_found(
    client,
    cancel_ticket_use_case,
):
    ticket_id = uuid4()

    cancel_ticket_use_case.execute.side_effect = TicketNotFoundException(
        ticket_id
    )
