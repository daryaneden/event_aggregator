...
from app.infrastructure.database.repositories.sqlalchemy_outbox_repository import (
    SqlAlchemyOutboxRepository,
)
from app.infrastructure.database.repositories.sqlalchemy_sync_state_repository import (
    SqlAlchemySyncStateRepository,
)
from app.infrastructure.database.repositories.sqlalchemy_ticket_repository import (
...
def get_sqlalchemy_sync_state_repository(session: Annotated[AsyncSession, Depends(get_db_session)]) -> SqlAlchemyEventRepository:
    return SqlAlchemySyncStateRepository(session)

def get_get_events_use_case(event_repository: Annotated[SqlAlchemyEventRepository, Depends(get_sqlalchemy_event_repository)]) -> GetEventsUseCase:
    return GetEventsUseCase(event_repository)

def get_event_response_mapper() -> EventResponseMapper:
    return EventResponseMapper()

def get_get_event_use_case(event_repository: Annotated[SqlAlchemyEventRepository, Depends(get_sqlalchemy_event_repository)]) -> GetEventUseCase:
...
def get_sync_events_use_case(provider: Annotated[EventsProviderClient, Depends(get_events_provider_client)],
...
def get_background_sqlalchemy_unit_of_work(event_repository: Annotated[SqlAlchemyEventRepository, Depends(get_sqlalchemy_event_repository)],
...
def get_get_available_seats_use_case(provider: Annotated[EventsProviderClient, Depends(get_events_provider_client)]) -> GetAvailableSeatsUseCase:
