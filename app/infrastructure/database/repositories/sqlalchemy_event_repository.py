...
class SqlAlchemyEventRepository:
    ...
    async def get_events(self, date_from: date | None,
                         page: int,
                         page_size: int) -> tuple[list[Event], int]:

        query = (select(EventModel).options(selectinload(EventModel.place)))
        ...
        query = (query.order_by(EventModel.event_time).offset((page - 1) * page_size).limit(page_size))

        result = await self.session.execute(query)

        event_models = result.scalars().all()

        events = [self._to_domain(event_model) for event_model in event_models]
    ...
    async def get_by_id(self, event_id: UUID) -> Event | None:
        query = (select(EventModel).options(selectinload(EventModel.place)).where(EventModel.id == event_id))
